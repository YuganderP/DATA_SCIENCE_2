...
async def lifespan(_app: FastAPI):
    async with engine.begin() as conn:
        ...
        yield  # where our applicaiton actually runs
        # shutdown
...
# changing the function to asyn
@app.get("/companies", response_model=list[CompanyResponse])

# changing the function to async
async def get_companies(db: AsyncSession = Depends(get_db)):
    companies = (
        await db.query(models.Company).options(selectinload(models.Company.name)).all()
    )
    return companies
...
def get_job_listings(db: Session = Depends(get_db)):
    data = db.query(models.JobListing).all()
    return data
